        ...
        try {
            ...
            const moodResult =
                await new Promise((resolve, reject) => {
                    dbPool.query(moodQuery, [user.user_id], (err, result) => {
                        if (err) reject(err);
                        if (result.length > 0) {
                            result.forEach(async (mood, index, array) => {

                                mood.emotion = await new Promise((resolve, reject) => {
                                    dbPool.query(emotionQuery, [user.user_id, mood.mood_id], (err, result) => {
                                        if (err) reject(err);
                                        resolve(result);
                                    });
                                });
                                ...
                                mood.context = await new Promise((resolve, reject) => {
                                    dbPool.query(contextQuery, [user.user_id, mood.mood_id], (err, result) => {
                                        if (err) reject(err);
                                        resolve(result);
                                    });
                                });
                                ...
                                array[index] = mood;
                                if (array.length - 1 == index) resolve(result)
                                if (array.length < 0) resolve(result);

                            });
                        } else { resolve(null) }
                    });
                });
            return moodResult.sort((a, b) => b.mood_timestamp - a.mood_timestamp)
        } catch (error) {
            console.log(error);
        }

let params = new URLSearchParams(document.location.search);
            ...
            function setLimit() {
                window.location.search = "?page=1&limit=" + recordLimit.value + "&sort=" + sort;
            }
            function setPage(page) {
                window.location.search = "?page=" + page + "&limit=" + limit + "&sort=" + sort;

            }function setSort() {
                    window.location.search = "?page=" + pg + "&limit=" + limit + "&sort=" + sorted.value;
            ...
            function prevPage() {
                currPg = parseInt(pg)
                if (currPg == 1) return;
                window.location.search = "?page=" + (currPg - 1) + "&limit=" + limit + "&sort=" + sort;
            }
            function nextPage() {
